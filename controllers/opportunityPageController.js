...
const { getDefaultOpportunityPageData } = require("../utils/defaultPageData"); // ✅ IMPORT from single source
...
// ✅ OPTIMIZED: Update Opportunity Page (Using shared defaults)
const UpdateOpportunityPage = async (req, res) => {
  upload(req, res, async (err) => {
    ...
    try {
      ...
      if (!existingPage) {
        const defaults = getDefaultOpportunityPageData(); // ✅ Use shared defaults
        const opportunityPageData = {
          tenant_id: tenantId,
          ...
          overview_section_youtube_url: overview_section_youtube_url || defaults.overview_section_youtube_url || null,
      ...
      // ✅ PAGE EXISTS - SMART UPDATE
      const opportunityPageData = {};

      if (hero_section_content !== undefined) opportunityPageData.hero_section_content = hero_section_content;
      if (description_section_content !== undefined) opportunityPageData.description_section_content = description_section_content;
      if (door_section_title !== undefined) opportunityPageData.door_section_title = door_section_title;
      if (door_section_content !== undefined) opportunityPageData.door_section_content = door_section_content;
      if (marketing_section_title !== undefined) opportunityPageData.marketing_section_title = marketing_section_title;
      if (marketing_section_content !== undefined) opportunityPageData.marketing_section_content = marketing_section_content;
      if (business_model_section_title !== undefined) opportunityPageData.business_model_section_title = business_model_section_title;
      if (business_model_section_content !== undefined) opportunityPageData.business_model_section_content = business_model_section_content;
      if (overview_section_title !== undefined) opportunityPageData.overview_section_title = overview_section_title;
      if (overview_section_youtube_url !== undefined) opportunityPageData.overview_section_youtube_url = overview_section_youtube_url || null;
      if (compensation_plan_section_title !== undefined) opportunityPageData.compensation_plan_section_title = compensation_plan_section_title;
      if (compensation_plan_section_content !== undefined) opportunityPageData.compensation_plan_section_content = compensation_plan_section_content;

      // Handle file uploads
      if (req.files) {
        if (req.files.hero_section_image) {
          const file = req.files.hero_section_image[0];
          if (existingPage.hero_section_image_url) await deleteFromS3(existingPage.hero_section_image_url);
          opportunityPageData.hero_section_image_url = await uploadToS3(file, `tenant_${tenantId}/opportunity_page_hero`);
          safeUnlink(file.path);
        }
        if (req.files.door_section_image) {
          const file = req.files.door_section_image[0];
          if (existingPage.door_section_image_url) await deleteFromS3(existingPage.door_section_image_url);
          opportunityPageData.door_section_image_url = await uploadToS3(file, `tenant_${tenantId}/opportunity_page_door`);
          safeUnlink(file.path);
        }
        if (req.files.marketing_section_image) {
          const file = req.files.marketing_section_image[0];
          if (existingPage.marketing_section_image_url) await deleteFromS3(existingPage.marketing_section_image_url);
          opportunityPageData.marketing_section_image_url = await uploadToS3(file, `tenant_${tenantId}/opportunity_page_marketing`);
          safeUnlink(file.path);
        }
        if (req.files.business_model_section_image) {
          const file = req.files.business_model_section_image[0];
          if (existingPage.business_model_section_image_url) await deleteFromS3(existingPage.business_model_section_image_url);
          opportunityPageData.business_model_section_image_url = await uploadToS3(file, `tenant_${tenantId}/opportunity_page_business`);
          safeUnlink(file.path);
        }
        if (req.files.overview_section_video) {
        ...
        if (req.files.compensation_plan_document) {
          const file = req.files.compensation_plan_document[0];
          if (existingPage.compensation_plan_document_url) await deleteFromS3(existingPage.compensation_plan_document_url);
          opportunityPageData.compensation_plan_document_url = await uploadToS3(file, `tenant_${tenantId}/opportunity_page_documents`);
          safeUnlink(file.path);
        }
      }
